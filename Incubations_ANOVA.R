...
# Null hypothesis: There is no significant difference in the creation of POC by different experimental groups
# Alternative hypothesis: There is a significant difference in sample means in at least one group compared to other groups
# If null is true, each group means are equal or not significantly different
# Ho: u_total_photosynthesis = u_DCMU = u_dark = u_Fe_spike

# Assumptions underlying ONE-WAY ANOVA
  # Assumption of independence: sample means in each group are independent of one another
...
ggdensity(Incubations_ANOVA$Normalized_C_mg_mL, 
          main = "Density Plot",
          xlab = "Groups")
# Q-Q plot draws a correlation between a given sample and the normal distribution. If all points fall approximately along the referenfce line, we can assume normality
...
# if p-value > 0.05, then the distribution of the data are not significantly different from normal distribution. Therefore, we can assume normality
shapiro.test(Incubations_ANOVA$Normalized_C_mg_mL)


### Assumption of Homogeneity of variance check using Bartlett Test
# If p-value is greater than significance level (0.05), there is no evidence to suggest that the variance is significantly different for the 4 treatment groups. the assumption of homogeneity of variance is met
bartlett.test(Normalized_C_mg_mL~Groups, Incubations_ANOVA)

##### ANOVA ANALYSIS #####
# Using this as reference: http://www.sthda.com/english/wiki/one-way-anova-test-in-r#check-your-data
...
# Groups      3 1.636e-06 5.452e-07   6.921  0.013 *
#  Residuals    8 6.302e-07 7.880e-08                 
#  Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
## Interpretation: As the p-value (listed here as Pr(>F)) is less than the significance level 0.05, we can conclude that there are significant differences between the groups highlighted with “*" in the model summary.
...
#     Fit: aov(formula = Normalized_C_mg_mL ~ Groups, data = Incubations_ANOVA)
#     $Groups
#      diff         lwr           upr          p adj
...
#Add Anova significance: there is a sig. dif between at least 2 groups
